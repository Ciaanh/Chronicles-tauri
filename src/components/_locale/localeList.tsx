  ...
  async function fetchLocales() {
    ...
    if (showOnlyUnreferenced) {
      filteredLocales = dbLocaleList.filter(
        (locale) =>
          typeof locale.id === "number" && !referencedIds.has(locale.id),
      );
    }

    const mappedLocales =
      await dbContext.mappers.locales.mapFromDbArray(filteredLocales);
  ...
  async function isLocaleReferenced(localeId: number): Promise<boolean> {
    ...
    const referencedInEvents = events.some((e) => {
      ...
      return (
        ...
        event.chapters.some(
          (chapter) =>
            chapter.headerId === localeId || chapter.pageIds.includes(localeId),
        )
      );
    });
    ...
    <Space direction="vertical" style={{ width: "100%" }}>
      <Space>
        ...
        <Space align="center">
          ...
          />
          <span>
            {showOnlyUnreferenced ? "Show Only Unreferenced" : "Show All"}
          </span>
        </Space>
      </Space>
