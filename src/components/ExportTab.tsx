...
const ExportTab: React.FC = () => {
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            const allCollectionsData = await dbContext.mappers.collections.mapFromDbArray(await dbContext.getAll(tableNames.collections));
            const allEvents = await dbContext.mappers.events.mapFromDbArray(await dbContext.getAll(tableNames.events));
            const allFactions = await dbContext.mappers.factions.mapFromDbArray(await dbContext.getAll(tableNames.factions));
            const allCharacters = await dbContext.mappers.characters.mapFromDbArray(await dbContext.getAll(tableNames.characters));

            // Filter to only include selected collections and their related data
            const selectedCollectionsData = allCollectionsData.filter(c => selectedCollections.includes(c.id));
            const selectedEvents = allEvents.filter(e => e.collection && selectedCollections.includes(e.collection.id));
            const selectedFactions = allFactions.filter(f => f.collection && selectedCollections.includes(f.collection.id));
            const selectedCharacters = allCharacters.filter(c => c.collection && selectedCollections.includes(c.collection.id));
            ...
            messageApi.success(`Export completed! Exported ${selectedCollectionsData.length} collection(s). ZIP file will be saved.`);
        } catch (e) {
        ...
        <div style={{ padding: 24 }}>
            ...
            <Typography.Paragraph>
                Generate and download the WoW Chronicles addon files (Lua/XML) for your selected collections.
            </Typography.Paragraph>
            ...
            <Card title="Select Collections to Export" style={{ marginBottom: 24 }}>
                <Space style={{ marginBottom: 16 }}>
                    ...
                    <Typography.Text type="secondary">
                        {selectedCollections.length} of {allCollections.length} collections selected
                    </Typography.Text>
                </Space>
