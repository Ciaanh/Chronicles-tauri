      ...
      for (const characterDb of filteredCharacters as DB_Character[]) {
        try {
          const mappedCharacter =
            await dbContext.mappers.characters.mapFromDb(characterDb);
      ...
      const processedChapters = await Promise.all(
        values.chapters?.map(async (chapter: any) => {
          const header = chapter.header
            ? await LocaleUtils.createOrUpdateLocale(chapter.header, dbContext)
            : null;

          const pages = await Promise.all(
            chapter.pages.map(
              async (page: any) =>
                await LocaleUtils.createOrUpdateLocale(page, dbContext),
            ),
          );
  ...
  return (
    <Space direction="vertical" style={{ width: "100%" }}>
      ...
      <CharacterModal
        ...
        characterToEdit={editingCharacter ? editingCharacter : undefined}
      />
    </Space>
  );
};
