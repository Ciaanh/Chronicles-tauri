...
export class Database {
  ...
  private getMaxId<T extends DbObject>(table: T[]): number | null {
    const maxId = table.length > 0 ? Math.max(...table.map((c) => c.id)) : null;
    return maxId;
  }
  ...
  public async insert<T extends DbObject>(
    ...
    if (this.tableExists(tablename, database)) {
      ...
      if (row.id === -1) {
        ...
        const nextId =
          maxId === null ? (this.schema.oneIndexed ? 1 : 0) : maxId + 1;
  ...
  public async getAll<T extends DbObject>(
    tablename: string,
    filter: ((value: T, index?: number, Array?: T[]) => boolean) | null = null,
  ): Promise<T[]> {
    const database = await this.loadDatabase();
